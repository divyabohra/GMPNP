...
This script solves the PNP/GMPNP system for a 3D cylindrical catalyst pore  \
for CO2ER at steady state. \
Output is concentration of solution species as well  \
...
def solveEDL(
    ...
    n_water = {
        'H': data['Hydration_number']['w_H'],
        cat_str: data['Hydration_number']['w_'+cat_str]
    }

    # storing solvated sizes of solution species
    solv_size = {'H': 0.0, 'OH': 0.0, 'HCO3': 0.0, 'CO32': 0.0,
                 'CO2': 0.0, 'CO': 0.0, 'H2': 0.0, cat_str: 0.0}
    ...
    # relative permittivity of water (electrolyte)
    eps_rel = data['nat_const']['eps_rel']
    ...
    z = {'H': 1, 'OH': -1, 'HCO3': -1, 'CO32': -2, 'CO2': 0,
         'CO': 0, 'H2': 0, cat_str: 1}
    ...
    scale_vol = {'H': 0.0, 'OH': 0.0, 'HCO3': 0.0, 'CO32': 0.0,
                 'CO2': 0.0, 'CO': 0.0, 'H2': 0.0, cat_str: 0.0}
    ...
    # boundaries markers analogous to S1, S2 and S3 in formulation doc
    ...
    # 8 concentrations and 1 potential elements
    element = MixedElement([P3, P3, P3, P3, P3, P3, P3, P3, P3])
    ...
    J_pore_exit_cat = Constant(J_prefactor[cat_str] * k_elec[cat_str] *
                               bulk_conc[cat_str]) * (u_cat - 1)
    ...
    F_H = ((u_H - u_nH) / (del_t)) * v_H * dx \
        ...
        + (
            ...
            + scale_vol['CO'] * grad(u_CO)
            + scale_vol['H2'] * grad(u_H2)
    ...
    F_cat = ((u_cat - u_ncat) / (del_t)) * v_cat * dx \
    ...
    F_p = - (
        eps_rel * (
            (55 - (n_water[cat_str] * u_cat * bulk_conc[cat_str]
                   + n_water['H'] * u_H * bulk_conc['H']) * 1.0e-3) / 55
        )
        + 6 * (
            ((n_water[cat_str] * u_cat * bulk_conc[cat_str]
              + n_water['H'] * u_H * bulk_conc['H']) * 1.0e-3) / 55
        )
    ) * dot(grad(u_p), grad(v_p)) * dx + (
        z['H'] * u_H * bulk_conc['H']
        + z['OH'] * u_OH * bulk_conc['OH']
        + z['HCO3'] * u_HCO3 * bulk_conc['HCO3']
        + z['CO32'] * u_CO32 * bulk_conc['CO32']
        + z[cat_str] * u_cat * bulk_conc[cat_str]
    ) * q * v_p * dx
    ...
    for n in range(tot_num_steps):
        ...
        _u_H, _u_OH, _u_HCO3, _u_CO32, _u_CO2, _u_CO, _u_H2, _u_cat, _u_p = \
            u.split()
    ...
    # estimating the electric field value as a function of x for the
    # last computed value of potential profile
    ...
    # saving unscaled values of concentrations, coordinates,
    # time and electric field in an npz
    np.savez(
    ...
    eps_rel_conc_ss = eps_rel * (
        (55 - (n_water[cat_str] * c_cat + n_water['H'] * c_H) * 1.0e-3) / 55
